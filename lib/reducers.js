...
import {
  ...
  REMOVE_SPRINT_ABSENCE, REMOVE_SPRINT_MEMBER,
...
function sprints (state = new List(), action) {
  ...
  if (action.type === REMOVE_SPRINT_ABSENCE) {
    ...
    return state.update(index, (sprint) => {
      const absenceIndex = sprint.get('absences').findIndex((absence) => {
        return absence.isSuperset(new Map({ reason, hours }));
      });
  ...
  if (action.type === REMOVE_SPRINT_MEMBER) {
    ...
    return state.update(index, (sprint) => {
      const teamIndex = sprint.get('team').indexOf(name);
      return sprint.deleteIn(['team', teamIndex]);
    });
  }
