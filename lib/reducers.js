...
function sprints (state = new List(), action) {
  ...
  if (action.type === SET_SPRINT_COMPLETED) {
    ...
    return state.setIn([index, 'points', 'completed'], number);
  }
  ...
  if (action.type === RECALC_SPRINT_AVERAGE) {
    ...
    return state.update(index, (sprint) => {
      const available = sprint.getIn(['teamHoursPerSprint', 'available']);
      const completed = sprint.getIn(['points', 'completed']);
